...
confirm_overwrite(const char *filename)
{
    ...
    if (yesno[0] != 'y' && yesno[0] != 'Y')
        return 0;
#else
    yesno = read_passphrase("Overwrite (y/n)? ", RP_ECHO);
    fprintf(thread_stdout, "Reply is: %s.\n", yesno);
    if (strlen(yesno) == 0) {
        free(yesno);
        return 0;
    }
...
#else
    char message[1024];
    snprintf(message, sizeof message, "%s (%s): ", prompt, identity_file);
    buf = read_passphrase(message, RP_ECHO);
    fprintf(thread_stdout, "Reply is: %s.\n", buf);
    fflush(stdout);
#endif
	buf[strcspn(buf, "\n")] = '\0';
...
do_convert_to(struct passwd *pw)
{
	...
	if (stat(identity_file, &st) == -1)
		fatal("%s: %s: %s", ssh_progname, identity_file, strerror(errno));
...
do_fingerprint(struct passwd *pw)
{
	...
	if (strcmp(identity_file, "-") == 0) {
		...
		fatal("%s: %s: %s", ssh_progname, path, strerror(errno));
...
do_change_comment(struct passwd *pw, const char *identity_comment)
{
	...
	if (comment)
		fprintf(thread_stdout, "Old comment: %s\n", comment);
	else
		fprintf(thread_stdout, "No existing comment\n");

	if (identity_comment) {
		strlcpy(new_comment, identity_comment, sizeof(new_comment));
	} else {
		fprintf(thread_stdout, "New comment: ");
		fflush(stdout);
	...
	if (comment != NULL && strcmp(comment, new_comment) == 0) {
		fprintf(thread_stdout, "No change to comment\n");
		free(passphrase);
		sshkey_free(private);
		free(comment);
		exit(0);
	...
	if (strlen(new_comment) > 0)
		fprintf(thread_stdout, "Comment '%s' applied\n", new_comment);
	else
		fprintf(thread_stdout, "Comment removed\n");
...
show_options(struct sshbuf *optbuf, int in_critical)
{
	...
	while (sshbuf_len(options) != 0) {
		...
		if (!in_critical &&
			...
			fprintf(thread_stdout, " UNKNOWN OPTION: %s (len %zu)\n",
			    hex, sshbuf_len(option));
			sshbuf_reset(option);
			free(hex);
		} else
			fprintf(thread_stdout, " UNKNOWN FLAG OPTION\n");
		free(name);
...
print_cert(struct sshkey *key)
{
	...
	fprintf(thread_stdout, "        Signing CA: %s %s (using %s)\n",
	...
	if (!quiet) {
		if (ret == 0) {
			if ((fp = sshkey_fingerprint(sign_key, fingerprint_hash,
				...
				fatal_f("sshkey_fingerprint failed");
			if (principal == NULL) {
				fprintf(thread_stdout, "Good \"%s\" signature with %s key %s\n",
				       sig_namespace, sshkey_type(sign_key), fp);

			} else {
				fprintf(thread_stdout, "Good \"%s\" signature for %s with %s key %s\n",
				       sig_namespace, principal,
				       sshkey_type(sign_key), fp);
			}
		} else {
			fprintf(thread_stdout, "Could not verify signature.\n");
		}
	}
	sshbuf_free(sigbuf);
	sshbuf_free(abuf);
...
do_download_sk(const char *skprovider, const char *device)
{
	...
	if (!quiet) {
		fprintf(thread_stdout, "You may need to touch your authenticator "
	...
	for (i = 0; i < nkeys; i++) {
		...
		if (!quiet) {
			fprintf(thread_stdout, "Saved %s key%s%s to %s\n",
			    sshkey_type(keys[i]),
			    *ext != '\0' ? " " : "",
...
save_attestation(struct sshbuf *attest, const char *path)
{
	...
	if (!quiet)
		fprintf(thread_stdout, "Your FIDO attestation certificate has been saved in "
		    "%s\n", path);
}
	...
	case KEY_ED25519_SK:
		...
		for (i = 0 ; ; i++) {
			...
			passphrase = read_passphrase("Enter PIN for "
			    "authenticator: ", RP_ALLOW_STDIN);
			if (!quiet) {
				fprintf(thread_stdout, "You may need to touch your "
