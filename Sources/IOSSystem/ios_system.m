...
static void initializeCommandList(void)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSError *error;
        // Try to find the bundle using a known class (if available) or main bundle
        NSBundle *bundle = [NSBundle mainBundle];

        NSString* commandDictionary = [bundle pathForResource:@"commandDictionary" ofType:@"plist"];
        if (!commandDictionary) {
             // Fallback to old behavior
            NSString* applicationDirectory = [[NSBundle mainBundle] resourcePath];
            commandDictionary = [applicationDirectory stringByAppendingPathComponent:@"commandDictionary.plist"];
        }
        ...
        if (sideLoading) {
            ...
            NSString* extraCommandsDictionary = [bundle pathForResource:@"extraCommandsDictionary" ofType:@"plist"];
            if (!extraCommandsDictionary) {
                NSString* applicationDirectory = [[NSBundle mainBundle] resourcePath];
                extraCommandsDictionary = [applicationDirectory stringByAppendingPathComponent:@"extraCommandsDictionary.plist"];
            }
