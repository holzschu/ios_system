                          ...
                          struct dynbuf *req)
{
  ...
  if(!data->state.disableexpect && Curl_use_http_1_1plus(data, conn) &&
    ...
    if(ptr) {
      ...
      result = Curl_dyn_addn(req, "Expect: 100-continue\r\n", sizeof("Expect: 100-continue\r\n") - 1);
      if(!result)
...
#define Curl_buffer_send(a,b,c,d,e,f) CURLE_OK
  ...
  case HTTPREQ_POST_MIME:
    ...
    if(conn->bits.authneg) {
      ...
      result = Curl_dyn_addn(r, "Content-Length: 0\r\n\r\n", sizeof("Content-Length: 0\r\n\r\n") - 1);
      if(result)
        return result;
  ...
  case HTTPREQ_POST:
    ...
    if(!Curl_checkheaders(data, STRCONST("Content-Type"))) {
      ...
      result = Curl_dyn_addn(r, "Content-Type: application/"
                                           "x-www-form-urlencoded\r\n", sizeof("Content-Type: application/"
                                                                                  "x-www-form-urlencoded\r\n") - 1);
      if(result)
        return result;
    }
      ...
      if(conn->httpversion < 20 &&
        ...
        if(!data->req.upload_chunky) {
          ...
          if(!result) {
            ...
            result = Curl_dyn_addn(r, "\x30\x0d\x0a\x0d\x0a", sizeof("\x30\x0d\x0a\x0d\x0a") - 1);
            /* 0  CR  LF  CR  LF */
            included_body += 5;
          }
        }
      ...
      while(co) {
        if(co->value) {
          if(0 == count) {
            ...
            result = Curl_dyn_addn(r, "Cookie: ", sizeof("Cookie: ") - 1);
            if(result)
              break;
          }
