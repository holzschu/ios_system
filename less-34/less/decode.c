...
init_cmds()
{
    // iOS: re-initialize the tables?
    memcpy(UNIX03cmdtable, UNIX03cmdtable_init, sizeof(UNIX03cmdtable_init));
    memcpy(cmdtable, cmdtable_init, sizeof(cmdtable_init));
    memcpy(edittable, edittable_init, sizeof(edittable_init));
...
/*
 * Cleanup the command lists
 */
    public void
clean_cmds()
{
    // Not enough.
    free(list_UNIX03cmd_tables);
    list_UNIX03cmd_tables = NULL;
    free(list_fcmd_tables);
    list_fcmd_tables = NULL;
    free(list_ecmd_tables);
    list_ecmd_tables = NULL;
    free(list_var_tables);
    list_var_tables = NULL;
    free(list_sysvar_tables);
    list_sysvar_tables = NULL;
}
/*
 * Add a command table.
 */
