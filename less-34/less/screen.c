...
// iOS: these functions are forbidden in the AppStore:
// extern char *tgetstr();
// extern char *tgoto();
	...
	int on;
{
	...
	if (on) 
	{
        ...
        erase_char = 0x7f; /* iOS: ^? for erase */
#ifdef VERASE2
		erase2_char = s.c_cc[VERASE2];
#endif
		// kill_char = s.c_cc[VKILL];
        kill_char = CONTROL('C');; /* iOS: ^c for kill char */
#ifdef VWERASE
		werase_char = s.c_cc[VWERASE];
#else
		werase_char = CONTROL('W');
#endif
        werase_char = CONTROL('W');; /* iOS: ^w for w erase */
	...
	char *capname;
{
	...
	if (hardcopy)
		return (0);
    // iOS: tgetflag is forbidden:
    if (strcmp(capname, "hc") == 0) // hardcopy (e.g. printer)
        return 0;
    if (strcmp(capname, "am") == 0)/* Terminal does \r\n when write past margin */
        return 1;
    if (strcmp(capname, "xn") == 0) /* Terminal ignores \n immediately after wrap */
        return 0;
    if (strcmp(capname, "da") == 0) // memory retained above screen
        return 1;
    if (strcmp(capname, "db") == 0) // memory retained below screen
        return 1;
    if (strcmp(capname, "ut") == 0)  /* Clear fills with background color */
        return 1;
    if (strcmp(capname, "bs") == 0) // backspace
        return 0;
    return 0;
	// return (tgetflag(capname));
}

/* iOS: can't use tgetnum
 static int
ltgetnum(capname)
	char *capname;
{
	...
	char **pp;
{
	...
	if (hardcopy)
		return (NULL);
    if (strcmp(capname, "ku") == 0) // up arrow
        return "\033[A";
    if (strcmp(capname, "kd") == 0) // down arrow
        return "\033[B";
    if (strcmp(capname, "kr") == 0) // right arrow
        return "\033[C";
    if (strcmp(capname, "kl") == 0) // left arrow
        return "\033[D";
    if (strcmp(capname, "kP") == 0) // page up
        return "\033[5;~";
    if (strcmp(capname, "kN") == 0) // page down
        return "\033[6;~";
    if (strcmp(capname, "kh") == 0) // home
        return "\033[1;~";
    if (strcmp(capname, "@7") == 0) // end
        return "\033[4;~";
    if (strcmp(capname, "kD") == 0) // delete
        return "\033[3;~";
    if (strcmp(capname, "kD") == 0) // delete
        return "\033[3;~";
    if (strcmp(capname, "ks") == 0) // start keypad mode
        return NULL;
    ...
    if (strcmp(capname, "cd") == 0) // Clear to end of screen
        return "\033[0J";
    if (strcmp(capname, "cl") == 0) // Clear screen
        return "\033[2J";
    if (strcmp(capname, "cm") == 0) // General cursor positioning
        return "\033[H";
    if (strcmp(capname, "vb") == 0) // Visual bell
        return NULL;
    if (strcmp(capname, "bs") == 0) // Backspace?
        return "\b";
    if (strcmp(capname, "ho") == 0) // Home
        return NULL;
    if (strcmp(capname, "ll") == 0) // lower-left
        return NULL;
    if (strcmp(capname, "cr") == 0) // carriage return
        return NULL;
    if (strcmp(capname, "al") == 0) // add line
        return NULL;
    if (strcmp(capname, "sr") == 0) // scroll reverse
        return "\033[1S";
    if (strcmp(capname, "pc") == 0) // Pad string
        return NULL;
    if (strcmp(capname, "bc") == 0) // backspace char
        return "\b"; 
    if (strcmp(capname, "so") == 0) // Enter standout (highlighted) mode
        return "\033[7m";
    if (strcmp(capname, "se") == 0) // Exit standout mode
        return "\033[0m";
    if (strcmp(capname, "us") == 0) // Enter underline mode
        return "\033[4m";
    if (strcmp(capname, "ue") == 0) // Exit underline mode
        return "\033[24m";
    if (strcmp(capname, "md") == 0) // Enter bold mode
        return "\033[1m";
    if (strcmp(capname, "me") == 0) // Exit bold mode
        return "\033[0m";
    if (strcmp(capname, "mb") == 0) // Enter blink mode
        return "\033[5m";
    // fprintf(stderr, "Called tgetstr %s\n", capname);
    return NULL;
...
get_term()
{
	...
	else
	{
        ...
        char gotobuf[10];
        sprintf(gotobuf, "\033[%d;1H", sc_height);
        strcpy(sp, gotobuf);
		t2 = sp;
		sp += strlen(sp) + 1;
	}
	...
	char *t;
{
    ...
    inc_costcount(t); // influence of sc_height?
	return (costcount);
}
