...
#define	DEBUG
#ifdef	DEBUG
#	define	DPRINTF(...)	if (dbg) fprintf(thread_stdout, __VA_ARGS__)
#else
#	define	DPRINTF(...)
#endif

extern __thread enum compile_states {
	RUNNING,
	COMPILING,
	ERROR_PRINTING
} compile_time;

extern __thread bool	safe;		/* false => unsafe, true => safe */
...
extern __thread bool	donefld;	/* true if record broken into fields */
extern __thread bool	donerec;	/* true if record is valid (no fld has changed */
extern __thread int	dbg;

extern __thread const char *patbeg;	/* beginning of pattern matched */
...
extern __thread Cell	*fsloc;		/* FS */
extern __thread Cell	*nfloc;		/* NF */
extern __thread Cell	*ofsloc;	/* OFS */
extern __thread Cell	*orsloc;	/* ORS */
extern __thread Cell	*rsloc;		/* RS */
extern __thread Cell	*rstartloc;	/* RSTART */
extern __thread Cell	*rlengthloc;	/* RLENGTH */
extern __thread Cell	*subseploc;	/* SUBSEP */
extern __thread Cell	*symtabloc;	/* SYMTAB */
...
typedef struct Node {
	...
	int	lineno;
	int	nobj;
	int nnarg;  /* iOS: required for cleanup */
